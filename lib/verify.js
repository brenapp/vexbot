...
function onServerJoin(member) {
    ...
    member.createDM().then(channel => {
        ...
        question("What should we call you? *(First Name, Nickname, etc.)*", channel)
            .then(name => verification.name = name).then(() => 
        questionValidate("What team are you *primarily* a part of?", async team => !!vexdb.size("teams", { team }), "That team doesn't exist! Sure that's the right team?", channel))
        ...
        ], channel)).then(role => verification.role = role)
        .then(async () => {
            ...
            switch(verification.role) {
                case "MEMBER": break; // No roles for just members! ;-)
                case "ALUMNI": roles.push("329760448020873229"); break; // Alumnus
                case "MENTOR": roles.push("329760518334054402"); break; // Mentor
            }
