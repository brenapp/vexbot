...
export default class PurgeCommand extends Command("purge") {
    ...
    async exec(message: Message, [count]: string[]) {
        ...
        // Get recent messages
        let messages = await channel.fetchMessages({ before: message.id, limit: 100 });

        // Run Filters
        if (filters.member) {
            messages = messages.filter(message => message.member.id != filters.member.id)
        }

        if (filters.role) {
            messages = messages.filter(message => message.member.roles.has(filters.role.id))
        }

        // Purge messages
        await Promise.all(messages.array().slice(0, +count).map(message => message.delete()));
