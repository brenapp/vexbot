...
class QualificationsCommand extends Command("quals") {
  ...
  documentation = {
    usage: "quals [region]",
    description:
      "Posts the state qualification leaderboard (region defaults to South Carolina)",
    group: "VEX",
  };
  ...
  async exec(message: Message, [region]: string[]) {
    ...
    const [awards, skills, quals] = await Promise.all([
      ...
      Promise.all(
        events.map((event) => vexdb.get("awards", { sku: event.sku }))
      ).then((a) => a.reduce((a, b) => a.concat(b), [])),

      // Get skills
      Promise.all(
        events.map((event) => vexdb.get("skills", { sku: event.sku, type: 2 }))
      ).then((a) => a.reduce((a, b) => a.concat(b), [])),
      ...
      Promise.all(
        events.map(({ sku }) =>
          html(
            ...
            .then(($) => (tableparse($), $))
            .then(($: any) =>
              $("#tab-awards .panel:last-child .panel-body table").parsetable(
                true,
                true,
                true
              )
            ...
            .then((quals) =>
              quals[0].filter(
                (v, i) => i > 0 && quals[1][i].includes("State/Regional")
              )
            )
        )
      ),
